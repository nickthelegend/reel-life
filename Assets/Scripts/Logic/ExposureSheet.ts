...
import { ShootRate, holdFrames, holdSeconds, steppedTime } from "./Stepped";

/**
 * The exposure sheet — the document stop-motion is actually planned on.
 *
 * An X-sheet is one row per film frame: which pose is exposed on it, how long
 * that pose is held, and where the beats fall. It is how animators think about
 * timing, and it is the natural readout for an app that shoots on twos.
 *
 * Everything here is derived from the clip and the shoot rate, so the sheet
 * always matches what playback will actually do.
 */

export interface XSheetRow {
  /** 1-based film frame, as an X-sheet is numbered. */
  frame: number;
  seconds: number;
  /** Which trimmed keyframe is showing on this frame. */
  keyframeIndex: number;
  /** True on the frame where a new pose is first exposed. */
  isNewExposure: boolean;
  /** Frames this exposure is held for. Only meaningful on its first frame. */
  heldFrames: number;
  /** True when this frame lands on a musical beat. */
  onBeat: boolean;
}
...
export interface XSheet {
  rows: XSheetRow[];
  fps: number;
  /** Film frames each exposure is held for; 0 when shooting smooth. */
  hold: number;
  totalFrames: number;
  exposures: number;
}
...
export function keyframeAt(clip: Clip, localT: number): number {
...
export function formatXSheet(sheet: XSheet, clipName: string): string {
  const header =
    `${clipName} — ${sheet.fps}fps` +
 ...
 */
export function nextExposureTime(
  clip: Clip,
  localT: number,
  rate: ShootRate
): number | null {
...
export function previousExposureTime(
  clip: Clip,
  localT: number,
  rate: ShootRate
): number | null {
