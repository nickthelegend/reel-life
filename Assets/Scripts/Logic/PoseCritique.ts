...
import { mirrorJointId, mirrorQuat } from "./PoseOps";
...
export function mirroredPairs(plan: RigPlan): Array<[string, string]> {
...
function isAtRest(q: { x: number; y: number; z: number; w: number }): boolean {
 ...
 */
export function twinningScore(pose: PoseSample, plan: RigPlan): number {
  let total = 0;
  let counted = 0;
...
export interface TwinningReport {
 ...
 */
export function detectTwinning(
  clip: Clip,
  plan: RigPlan,
  threshold: number = TWINNING_THRESHOLD
): TwinningReport[] {
  const frames = trimmedKeyframes(clip);
  const pairs = mirroredPairs(plan);
  const out: TwinningReport[] = [];
 ...
 */
export function poseContrast(a: PoseSample, b: PoseSample, plan: RigPlan): number {
...
export function lowContrastPairs(
  clip: Clip,
  plan: RigPlan,
  threshold: number = LOW_CONTRAST_CM
): ContrastReport[] {
  const frames = trimmedKeyframes(clip);
...
// ---------------------------------------------------------------------------
//
// A note on line-of-action scoring, which is NOT here.
//
// The obvious third check is whether the spine reads as one flowing curve.
// It was built, and then removed: this rig places `torso` at the same point as
// `hips` and `head` at the same point as `neck`, so the spine resolves to only
// two distinct world positions. A two-point chain is straight by definition, so
// the metric returned "stiff" for every possible pose — a coaching note that
// fires 100% of the time regardless of what the user does is worse than no note
// at all. It becomes meaningful only if the rig gains real spine offsets.
//
// ---------------------------------------------------------------------------

export interface CritiqueSummary {
  twinnedKeyframes: number[];
  lowContrastPairs: ContrastReport[];
}

export function critiqueClip(clip: Clip, plan: RigPlan): CritiqueSummary {
  return {
