...
import { EaseName, easeByName } from "./Easing";
import { PoseKeyframe, PoseSample, clonePose } from "./PoseTypes";
import { clamp, qSlerp, v3Lerp } from "./Vec";
...
function findSegment(frames: PoseKeyframe[], t: number): number {
...
export function blendPoses(a: PoseSample, b: PoseSample, u: number): PoseSample {
...
export function nearestKeyframeIndex(clip: Clip, localT: number): number {
  const frames = trimmedKeyframes(clip);
  if (frames.length === 0) {
    return -1;
  }
