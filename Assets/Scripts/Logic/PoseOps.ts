...
import { PoseSample, clonePose } from "./PoseTypes";
import { Quat, Vec3, qNormalize, qSlerp, v3Lerp } from "./Vec";
 ...
 */
export function mirrorJointId(jointId: string): string {
  if (jointId.length < 2) {
    return jointId;
  }
  const swapSuffix = (suffix: string, other: string): string | null => {
...
export function mirrorPose(pose: PoseSample): PoseSample {
...
export function mirrorClip(clip: Clip, newId: string): Clip {
 ...
 */
export function smoothClip(
  clip: Clip,
  newId: string,
  strength = DEFAULT_SMOOTH_STRENGTH,
  passes = DEFAULT_SMOOTH_PASSES
): Clip {
  const out = cloneClip(clip, newId);
 ...
 */
export function jitterScore(clip: Clip, jointId: string): number {
  let total = 0;
 ...
 */
export function shouldSuggestSmoothing(clip: Clip, jointIds: string[]): boolean {
...
export function replaceKeyframePose(clip: Clip, index: number, pose: PoseSample): boolean {
...
export function copyKeyframePose(clip: Clip, index: number): PoseSample | null {
  if (index < 0 || index >= clip.keyframes.length) {
    return null;
  }
