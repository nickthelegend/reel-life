...
Assets/Scripts/
  ...
  Logic/ (cont.)  AudioDirector, PanelLayout, CaptionFade, PlacementStability,
                  PartScaling, SampleGate — decision logic extracted out of the
                  engine shell so it can actually be run and tested
Assets/Audio/     four scored tracks + three foley one-shots, generated by
                  tools/build-audio.mjs (real WAVs, exact BPM)
typings/          vendored subset of the Lens Studio / SIK / RSG APIs, for
                  offline typechecking only — the editor never sees these
harness/          browser harness that runs the real Logic modules (see below)
tests/            290 tests over the Logic layer, incl. a full flow rehearsal
```
...
declarations, then the 290-test suite. `tests/flow.test.ts` is the end-to-end
...
The `Logic/` layer — rig planning, interpolation, stepping, follow-through,
timeline editing, retargeting, persistence, audio direction — runs unmodified
in a browser:

```bash
npm install && npx http-server . -p 8123
```

Then open `http://localhost:8123/harness/index.html`. It imports the same source
files the Lens compiles; nothing is reimplemented. Persistence goes through the
real `ReelStore` against `localStorage`, and the four scored tracks play.
...
- **Verified here:** the whole `Logic/` layer (290 tests, all passing), 112
  browser checks against a running harness, and a typecheck of every Lens
  source file. See
