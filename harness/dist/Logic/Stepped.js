 ...
 */
export function steppedTime(localT, rate) {
...
export function describeShootRate(rate) {
    if (rate.mode === "smooth") {
        return "Smooth";
    }
    return `On ${rate.mode} · ${Math.round(effectiveFps(rate))}fps`;
}
export function nextShootMode(mode) {
